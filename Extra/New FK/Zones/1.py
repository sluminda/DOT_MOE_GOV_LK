...
csv_file_path = 'zone_List.csv'
sql_file_path = 'zone.sql'
...
with open(csv_file_path, mode='r', encoding='utf-8') as file:
    ...
    insert_statement = "INSERT INTO zone (zonecode, zonename, distcode, zonename_si, zonename_ta, status) VALUES "
    ...
    for row in csv_reader:
        values = ", ".join(f"'{value}'" if i != 5 else value for i, value in enumerate(row))  # Ensure status is not enclosed in quotes
